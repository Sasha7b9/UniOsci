...
void Governor_NextPosition(Governor *governor)
{
    ...
    {
        CircleIncrease<int8>(&gCurDigit, 0, Governor_NumDigits(   governor) - 1);
    }
}
...
{
    CircleIncrease<int8>(&gCurDigit, 0, ipEthernet_IP->port == 0 ? 11 : 16);
}
...
{
    CircleIncrease<int8>(time->curField, 0, 7);
    painter.ResetFlash();
}
