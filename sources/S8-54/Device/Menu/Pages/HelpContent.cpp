...
static void DrawPageContent(void)
{
    ...
    while(currentPage->pages[numPage])
    {
        ...
        const char *title = page->s->titleHint[LANG];
...
void HelpContent_Draw(void)
{
    ...
    painter.DrawRectangle(1, 0, WIDTH + 1, 239, gColorFill);   /** \todo Здесь непонятно, почему так. Координаты верхнего левого угла должны быть 
