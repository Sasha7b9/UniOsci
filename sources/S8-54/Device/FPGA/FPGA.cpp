    ...
    *WR_ADDR_NSTOP = 0xffff;
    ...
    if (SET_INVERSE(ch))
    {
        while (data <= last)
        {
            ...
            *data = (uint8)((int)(2 * AVE_VALUE) - _math.LimitationUInt8((uint8)newData, MIN_VALUE, MAX_VALUE));
            data += step;
        }
    }
    else
...
static void InverseDataIsNecessary(Channel ch, uint8 *data)
{
    if (SET_INVERSE(ch))
    {
        ...
        {
            data[i] = (uint8)((int)(2 * AVE_VALUE) - _math.LimitationUInt8(data[i], MIN_VALUE, MAX_VALUE));
        }
    }
}
...
void FPGA_FindAndSetTrigLevel(void)
{
    ...
    uint8 min = _math.GetMinFromArray_RAM(data, 0, lastPoint);
    uint8 max = _math.GetMaxFromArray_RAM(data, 0, lastPoint);
