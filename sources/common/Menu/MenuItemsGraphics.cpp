...
void GovernorColor::DrawOpened(int x, int y)
{
    ...
    Painter::DrawVolumeButton(x + 1, y + 1, MI_WIDTH_VALUE + 2 + delta, MI_HEIGHT_VALUE + 3, 2, Color::MenuItem(false),
                             COLOR_MENU_ITEM_BRIGHT, COLOR_MENU_ITEM_DARK, IsPressed(), IsShade());
...
void Governor::DrawValue(int x, int y)
{
    ...
    Painter::DrawText(x + 55, y - 5, Int2String(maxValue, sign, 1, buffer), COLOR_WHITE);
...
void Governor::DrawLowPart(int x, int y, bool, bool shade)
{
    ...
    Painter::DrawVolumeButton(x + 1, y + 17, MI_WIDTH_VALUE + 2, MI_HEIGHT_VALUE + 3, 2, COLOR_MENU_FIELD,
                             COLOR_MENU_ITEM_BRIGHT, COLOR_MENU_ITEM_DARK, true, shade);
    if (shade)
    {
    ...
    {
        x = Painter::DrawText(x + 1, y + 21, Int2String(*cell, false, 1, buffer), COLOR_WHITE);
    }
...
void MACaddress::DrawValue(int x, int y)
{
    ...
    for (int i = 5; i >= 0; --i)
    {
        ...
        Painter::DrawText(x, y, buffer, gCurDigit == i ? COLOR_BLACK : COLOR_WHITE);
        x -= 12;
    }
}
...
void Choice::DrawOpened(int x, int y)
{
    ...
    Painter::DrawVolumeButton(x, y + MOI_HEIGHT_TITLE, MOI_WIDTH - 1, height - MOI_HEIGHT_TITLE, 1, COLOR_BLACK, COLOR_MENU_TITLE_BRIGHT,
                             COLOR_MENU_TITLE_DARK, false, IsShade());
    int8 index = *cell;
    for (int i = 0; i < NumSubItems(); i++)
    {
        ...
        if (pressed)
        {
            Painter::DrawVolumeButton(x + 1, yItem, MOI_WIDTH - 2, MOSI_HEIGHT - 2, 2, COLOR_MENU_FIELD, COLOR_MENU_TITLE_BRIGHT,
                                     COLOR_MENU_TITLE_DARK, pressed, IsShade());
        }
        Painter::DrawText(x + 4, yItem + 2, NameSubItem(i), pressed ? COLOR_BLACK : COLOR_MENU_FIELD);
    }
}
...
void Choice::DrawClosed(int x, int y)
{
    ...
    Painter::DrawVolumeButton(x + 1, y + 17, MI_WIDTH_VALUE + 2, MI_HEIGHT_VALUE + 3, 1, COLOR_MENU_FIELD, COLOR_MENU_ITEM_BRIGHT,
                             COLOR_MENU_ITEM_DARK, true, shade);

    int deltaY = (int)Step();
    Color colorText = shade ? Color::MenuItem(true) : COLOR_BLACK;
...
void Button::Draw(int x, int y, bool)
{
    ...
    Color color = shade ? Color::MenuItem(true) : COLOR_WHITE;
    Painter::FillRegion(x + 1, y + 2, MI_WIDTH - 2, MI_HEIGHT - 2, Color::MenuItem(false));
    Painter::DrawVolumeButton(x + 3, y + 4, MI_WIDTH - 6, MI_HEIGHT - 6, 2, Color::MenuItem(false), COLOR_MENU_ITEM_BRIGHT,
                             COLOR_MENU_ITEM_DARK, pressed, shade);
...
void Time::DrawClosed(int x, int y)
{
    ...
    Painter::DrawText(x + startX, y, Int2String((int)time.hours, false, 2, buffer), shade ? Color::MenuItem(true) : COLOR_BLACK);
...
void Time::DrawOpened(int x, int y)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        Painter::DrawText(x + strPaint[i].x, y + strPaint[i].y, strI[i], *curField == i ? COLOR_FLASH_01 : COLOR_WHITE);
    }
}
...
static void DrawGovernorChoiceColorFormulaHiPart(Control *item, int x, int y, bool pressed, bool shade, bool opened)
{
    ...
    Color color = shade ? COLOR_MENU_TITLE_DARK : (COLOR_SCHEME_IS_WHITE_LETTERS ? COLOR_WHITE :COLOR_BLACK);
    Painter::DrawHLine(y + 1, x, x + width + 3, Color::BorderMenu(false));

    Painter::DrawVolumeButton(x + 1, y + 2, width + 2, MI_HEIGHT_VALUE + 3, 1, Color::MenuItem(false), COLOR_MENU_ITEM_BRIGHT, COLOR_MENU_ITEM_DARK, 
        pressed, shade);
    ...
    if(Menu::CurrentItem() == item)
    {
        ...
        Painter::Draw4SymbolsInRect(x + MI_WIDTH - 13, y + 5 + (item->IsOpened() ? 0 : 15), symbol, shade ? color : COLOR_BLACK);
    }
}
